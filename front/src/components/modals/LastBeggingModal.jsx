...
const LastBeggingModal = ({ isOpen, onClose, onConfirm, message, subMessage }) => {
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} className="min-h-[280px]">
      <div className="flex flex-col items-center justify-center w-full h-full py-6">
        {/* 전달받은 메시지를 줄바꿈(\n) 처리하여 출력 */}
        <p className="text-2xl font-black text-white text-center leading-relaxed whitespace-pre-line mb-4">
          {message}
        </p>

        {/* 규칙 설명 (subMessage가 있을 때만 렌더링) */}
        {subMessage && (
          <div className="mt-0 mb-6 bg-black/20 p-4 rounded-xl border border-white/5">
            <p className="text-sm text-white/60 leading-relaxed text-left whitespace-pre-line">
              {subMessage}
            </p>
          </div>
        )}
        
        {/* 서브 메시지가 없을 때를 대비한 여백 조절 */}
        {!subMessage && <div className="mb-10" />}
