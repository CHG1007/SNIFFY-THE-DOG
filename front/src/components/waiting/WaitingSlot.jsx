...
const WaitingSlot = ({ player, isMe, isHost, onKickRequest }) => {
  const currentAudioLevel = player.isMicOn ? (player.audioLevel ?? 0) : 0;
  const isSpeaking = currentAudioLevel > 20;
    ...
    `}>
      ...
      {player.ready && (
        <div className="absolute top-4 left-4 z-30 flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="bg-primary text-white px-3 py-1 rounded-full shadow-lg flex items-center justify-center font-bold text-[10px] tracking-wider uppercase backdrop-blur-sm border border-primary/30">
            <span>READY</span>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-black via-black/80 to-transparent flex items-end justify-between px-5 pb-4 z-20">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col justify-end">
            ...
            {isMe && <span className="text-[10px] text-primary font-semibold tracking-wider uppercase mt-1">You</span>}
          </div>
        </div>
